...
module.exports.getPUBGvideos = function (cb, next){
    pool.getConnection(function (err, conn) {
        if (err) {
            cb(err, { code: 500, status: "Error connecting to database." })
            return;
        }
            conn.query("SELECT Video.videoTitle, Video.videoDescription, Video.rating, Video.reference, User.username FROM Video INNER JOIN User on Video.userID = User.userID WHERE Video.game = 'PUBG' ORDER BY Video.rating", function (err, results) {
                conn.release();
                if (err) {
